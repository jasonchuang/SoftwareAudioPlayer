...
//    private String TAG = "VideoSurfaceView";
    ...
    private AudioFrameGrabberAsyncTask mAudioFrameGrabberAsyncTask;
    ...
    // mCurrentState is a VideoSurfaceView object's current state.
    // mTargetState is the state that a method caller intends to reach.
    // For instance, regardless the VideoSurfaceView object's current state,
    ...
    public native int nativePrepareByteBuffer(ByteBuffer byteBuffer);
    ...
    public void Putarray(byte[] array) {
        android.util.Log.d("jason", "in java layer:");
    }

    private void jniFromCToJava(int test) {
        android.util.Log.d("jason", "in java layer:" + test);
    }
...
//    }
            ...
            public boolean onError(MediaPlayer mp, int framework_err, int impl_err) {
                ...
                if (getWindowToken() != null) {
                    ...
                    if (framework_err == MediaPlayer.MEDIA_ERROR_NOT_VALID_FOR_PROGRESSIVE_PLAYBACK) {
                        messageId = R.string.VideoSurfaceView_error_text_invalid_progressive_playback;
                    } else {
                        messageId = R.string.VideoSurfaceView_error_text_unknown;
                    }

                    new AlertDialog.Builder(mContext)
                        .setMessage(messageId)
                        .setPositiveButton(R.string.VideoSurfaceView_error_button,
     ...
     * or if the listener returned false, VideoSurfaceView will inform
...
//            openVideo();
    ...
    private void stopVideoPlayTask() {
        if (mAudioFrameGrabberAsyncTask != null) {
            mAudioFrameGrabberAsyncTask.cancel(false);
        }
    }
    ...
    @Override
    public void start() {
        android.util.Log.d("jason", "start AudioFrameGrabberAsyncTask");
        mAudioFrameGrabberAsyncTask = new AudioFrameGrabberAsyncTask();
        mAudioFrameGrabberAsyncTask.execute(mPath);
//        mMediaController.show();
    }
...
//        return mAudioSession;
    ...
    class AudioFrameGrabberAsyncTask extends AsyncTask<String, Void, Boolean> {
        private String data;

        public AudioFrameGrabberAsyncTask() {
        }
        ...
        protected Boolean doInBackground(String... params) {
            data = params[0];
            int init = nativeMediaInit(data);
            if (init < 0) {
                Log.d(TAG, "init failed, just return");
                return false;
            }

        android.util.Log.d("jason", "doInBackground data:" + data);

            int sampleRateInHz = nativeGetSampleRate();
            int channelConfig = nativeGetChannelCount() > 1 ? AudioFormat.CHANNEL_OUT_STEREO
            ...
            nativePrepareByteBuffer(byteBuffer);

            audioTrack.play();
            int currentFrame = 0;
            while (true) {
                long startMs = System.currentTimeMillis();
                currentFrame = nativeGetFrame();
            Log.d(TAG, "byteBuffer.remainging:" + byteBuffer.remaining());
            Log.d(TAG, "byteBuffer.position:" + byteBuffer.position());
            Log.d(TAG, "byteBuffer.capacity:" + byteBuffer.capacity());


            Log.d(TAG, "currentFrame:" + currentFrame);
                if (currentFrame > 0) {
                    byte[] output= new byte[currentFrame];
                    byteBuffer.get(output);
                    byteBuffer.clear();
//                    byteBuffer.position(0);
            Log.d(TAG, "before written:");
                    int written = audioTrack.write(output, 0, currentFrame);
        ...
        protected void onPostExecute(Boolean result) {
            if (!result) {
                int messageId = R.string.VideoSurfaceView_error_text_unknown;
                new AlertDialog.Builder(mContext)
                    .setMessage(messageId)
                    .setPositiveButton(R.string.VideoSurfaceView_error_button, null)
                    .setCancelable(false)
                    .show();
            }
        }
    ...
    public void setVideoSurfaceBitmap() {
        configureBounds();
        drawBitmap();
    }
